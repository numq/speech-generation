...
static void bark_print_statistics(gpt_model *model) {
    ...
    printf("%s:  predict time = %8.2f ms / %.2f ms per token\n", __func__, model->t_predict_us / 1000.0f,
           model->t_predict_us / model->n_sample / 1000.0f);
...
static bool ggml_quantize_weights(
    ...
    for (int i = 0; i < n_tensors; i++) {
        ...
        printf("%64s - [%5d, %5d, %5d], type = %6s ", name.data(), ne[0], ne[1], ne[2],
        ...
        if (quantize) {
            ...
            switch ((ggml_type) ttype) {
                ...
                case GGML_TYPE_Q5_K:
                case GGML_TYPE_Q6_K: {
                    cur_size = ggml_quantize_chunk((ggml_type) ttype, data_f32.data(), work.data(), 0,
                                                   nelements / ne[0], ne[0], nullptr);
                }
                    break;
                case GGML_TYPE_F32:
                case GGML_TYPE_F16:
...
static struct ggml_cgraph *bark_build_gpt_graph(
    ...
    static size_t buf_size =
            ggml_tensor_overhead() * BARK_MAX_NODES + ggml_graph_overhead_custom(BARK_MAX_NODES, false);
    ...
    if (*n_past > 0) {
        ...
        if (merge_ctx) {
            ...
            struct ggml_tensor *ctx_embd = ggml_get_rows(ctx0, model->wtes[0], ggml_view_1d(ctx0, input, 256, 256 *
                                                                                                              ggml_element_size(
                                                                                                                      input)));
            struct ggml_tensor *rem_embd = ggml_get_rows(ctx0, model->wtes[0],
                                                         ggml_view_1d(ctx0, input, 1, 512 * ggml_element_size(input)));
            ...
            tok_emb = ggml_set_1d(ctx0, tok_emb, rem_embd,
                                  cat_emb->ne[0] * cat_emb->ne[1] * ggml_element_size(cat_emb));
        } else {
            tok_emb = ggml_get_rows(ctx0, model->wtes[0], input);
        }
    }
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            if (N >= 1) {
                struct ggml_tensor *k = ggml_view_1d(ctx0, model->memory_k, N * n_embd,
                                                     (ggml_element_size(model->memory_k) * n_embd) *
                                                     (il * n_ctx + *n_past));
                struct ggml_tensor *v = ggml_view_1d(ctx0, model->memory_v, N * n_embd,
            ...
            struct ggml_tensor *V_trans =
                    ggml_cont(ctx0,
                              ggml_permute(ctx0,
                                           ggml_cpy(ctx0,
                                                    Vcur,
                                                    ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, n_embd / n_head, n_head,
                                                                       N)),
                                           1, 2, 0, 3));
...
static bool bark_eval_fine_encoder(struct bark_context *bctx, int n_threads) {
    ...
    for (int n = 0; n < n_loops; n++) {
        ...
        for (int nn = n_coarse; nn < n_fine_codebooks; nn++) {
            if (params.progress_callback) {
                const int progress_cur = 100 * (n * (n_fine_codebooks - n_coarse) + (nn - n_coarse + 1)) /
...
bool bark_forward_fine_encoder(struct bark_context *bctx, int n_threads) {
    ...
    // allocate the compute buffer
    {
        ...
        struct ggml_cgraph *gf = bark_build_fine_gpt_graph(&model, decoy_tokens, 2 /* codebook_idx */,
                                                           n_fine_codebooks);
